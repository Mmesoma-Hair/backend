...
def test_upload_via_backend_stores_public_id() -> None:
    ...
    assert image.public_id.startswith("idealcommerce/products/")


@pytest.mark.django_db
def test_add_image_from_existing_public_id() -> None:
    product = ProductFactory()
    image = services.add_product_image(product, public_id="idealcommerce/products/abc")
    assert image.public_id == "idealcommerce/products/abc"


def test_delivery_urls_use_auto_format_and_quality() -> None:
    urls = image_urls("idealcommerce/products/abc")
    assert set(urls) == {"thumb", "card", "detail"}
    assert "f_auto,q_auto" in cloudinary_url("idealcommerce/products/abc", "card")


@pytest.mark.django_db
def test_deleting_image_enqueues_cloudinary_cleanup() -> None:
    product = ProductFactory()
    image = services.add_product_image(product, public_id="idealcommerce/products/to-delete")
    ...
    with mock.patch("apps.catalog.signals.delete_cloudinary_asset.delay") as delay:
        image.delete()
    delay.assert_called_once_with("idealcommerce/products/to-delete")


@pytest.mark.django_db
def test_deleting_product_cleans_up_its_images() -> None:
    product = ProductFactory()
    services.add_product_image(product, public_id="idealcommerce/products/p1")
    services.add_product_image(product, public_id="idealcommerce/products/p2")
    with mock.patch("apps.catalog.signals.delete_cloudinary_asset.delay") as delay:
        product.delete()
