# Deployment — Eandewigs API (eandewigs-api)
...
`/home/mmeso/backend` owned by the unprivileged `mmeso` user.
...
| `nginx.conf`           | `/etc/nginx/sites-available/eandewigs-api`     | Reverse proxy + static (certbot adds TLS) |
| `sudoers.d-mmeso`      | `/etc/sudoers.d/mmeso-deploy`               | Passwordless service restart for deploys |
...
`/home/mmeso/backend/.env` (loaded by python-dotenv). The `.env` is **never**
...
# user
adduser --disabled-password --gecos "" mmeso && usermod -aG sudo mmeso
install -d -m 700 -o mmeso -g mmeso /home/mmeso/.ssh
# (paste deploy_key.pub into /home/mmeso/.ssh/authorized_keys)

# uv (as mmeso)
sudo -iu mmeso bash -c 'curl -LsSf https://astral.sh/uv/install.sh | sh'
```

Postgres (remote access was requested — keep the password strong):
```bash
sudo -u postgres psql -c "CREATE USER eandewigs WITH PASSWORD '<STRONG>';"
sudo -u postgres psql -c "CREATE DATABASE eandewigs OWNER eandewigs;"
# listen on all interfaces + allow remote auth (see Security note below)
```

App:
```bash
sudo -iu mmeso git clone https://github.com/Mmesoma-Hair/backend.git ~/backend
# write ~/backend/.env (see .env.example), then:
sudo -iu mmeso bash -lc 'cd ~/backend && uv sync --frozen --no-dev \
...
install -m 0440 deploy/sudoers.d-mmeso /etc/sudoers.d/mmeso-deploy
systemctl daemon-reload
systemctl enable --now gunicorn celery-worker celery-beat

# nginx + TLS
cp deploy/nginx.conf /etc/nginx/sites-available/eandewigs-api
ln -sf /etc/nginx/sites-available/eandewigs-api /etc/nginx/sites-enabled/
...
- **Automatic:** push to `main` → GitHub Actions (`.github/workflows/deploy.yml`)
  scps the code to the server as `mmeso`, then installs, migrates and restarts.
- **Manual:** `ssh mmeso@api.eandewigs.com 'cd ~/backend && bash deploy/deploy.sh'`
...
- The `mmeso` deploy sudo rule is scoped to three `systemctl` commands only.
